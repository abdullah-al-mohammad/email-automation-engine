...
export function TriggerNode({ data }: { data: { trigger: WorkflowTriggerResponse; isFirst: boolean } }) {
  const { trigger } = data;
    ...
    <div className="w-[280px] bg-white dark:bg-zinc-900 border-2 border-indigo-500 rounded-xl shadow-sm px-4 py-3">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-zinc-400 truncate capitalize">{label.replace('_', ' ')}</p>
        </div>
      </div>
      <Handle
        type="source"
