...
import { type WorkflowStepResponse, SUPPORTED_STEP_ACTIONS } from '@email-automation-engine/shared';
...
export function StepNode({ data }: { data: { step: WorkflowStepResponse } }) {
  const { step } = data;
  
  const isSplit = step.action === 'conditional_split';
  ...
  return (
    <div className="w-[280px] bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl shadow-sm px-4 py-3 group hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h4 className="text-sm font-medium text-gray-900 dark:text-white capitalize">{label.replace('_', ' ')}</h4>
          <p className="text-xs text-gray-500 dark:text-zinc-400 truncate">
            {step.action === 'delay' ? `${(step.config as any)?.durationValue} ${(step.config as any)?.durationUnit}` : 'Configure action'}
          </p>
        </div>
      </div>
      ...
      {isSplit ? (
        <>
          ...
          />
          <div className="absolute -bottom-6 left-1/4 transform -translate-x-1/2 text-[10px] font-bold text-green-600 uppercase">True</div>
          <div className="absolute -bottom-6 right-1/4 transform translate-x-1/2 text-[10px] font-bold text-red-600 uppercase">False</div>
        </>
      ) : (
        <Handle
          type="source"
