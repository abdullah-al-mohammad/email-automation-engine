...
export default function WorkflowContactsPage() {
  ...
  const { data: contacts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<ContactWorkflowResponse[]>(`/tenants/${currentTenant?.id}/workflows/${workflowId}/contact-workflows`);
      return res.data;
    },
      ...
      <div className="flex items-center justify-between border-b border-gray-200 dark:border-zinc-800 pb-5">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link to={`/workflows/${workflowId}`} className="text-sm font-medium text-indigo-600 hover:text-indigo-700">
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-zinc-800">
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="px-6 py-4 text-center text-gray-500">No contacts have entered this workflow yet.</td></tr>
            ) : (
              contacts.map(cw => (
                <tr key={cw.id} className="hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                  <td className="px-6 py-4 text-gray-900 dark:text-zinc-300 font-mono text-xs">{cw.contactId}</td>
                  <td className="px-6 py-4 text-gray-900 dark:text-zinc-300">{cw.triggerEvent}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium capitalize ${
                      cw.status === 'finished' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
                      cw.status === 'error' ? 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' :
...
function TimelineModal({ isOpen, onClose, contactWorkflowId, workflowId }: { isOpen: boolean; onClose: () => void; contactWorkflowId: string; workflowId: string }) {
  const { currentTenant } = useTenant();

  const { data: timeline = [], isLoading } = useQuery({
    queryKey: ['workflow-timeline', currentTenant?.id, workflowId, contactWorkflowId],
    queryFn: async () => {
      const res = await api.get<ContactWorkflowStepResponse[]>(`/tenants/${currentTenant?.id}/workflows/${workflowId}/contact-workflows/${contactWorkflowId}`);
      return res.data;
    },
                ...
                {timeline.map((step, idx) => (
                  <div key={step.id} className="relative pl-6">
                    <span className={`absolute -left-2.5 top-1 flex h-5 w-5 items-center justify-center rounded-full ring-4 ring-white dark:ring-zinc-900 ${
                      ...
                      step.status === 'scheduled' ? 'bg-yellow-500' : 'bg-gray-300 dark:bg-zinc-600'
                    }`} />
                    <div>
                      <div className="text-sm font-medium text-gray-900 dark:text-white mb-1">
                        Step ID: <span className="font-mono text-xs text-gray-500">{step.workflowStepId}</span>
                      </div>
                      <div className="text-xs text-gray-500 dark:text-zinc-400 capitalize mb-1">
                        Status: <span className="font-semibold text-gray-700 dark:text-zinc-300">{step.status}</span>
                      </div>
                      {step.scheduledAt && (
