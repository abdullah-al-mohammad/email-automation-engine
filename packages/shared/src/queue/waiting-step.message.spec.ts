...
describe('waitingStepMessageSchema', () => {
  ...
  it('accepts valid messages', () => {
    expect(waitingStepMessageSchema.safeParse(validMessage).success).toBe(true);
  });
  ...
  it('type guard returns true for valid', () => {
    expect(isWaitingStepMessage(validMessage)).toBe(true);
  });

  it('type guard returns false for invalid', () => {
    const invalid = { ...validMessage, version: 2 };
    expect(isWaitingStepMessage(invalid)).toBe(false);
  });
});
